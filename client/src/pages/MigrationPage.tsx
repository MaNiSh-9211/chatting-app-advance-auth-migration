...
import { LockIcon, MailIcon, CheckIcon, XIcon } from '../components/Icons';
...
export const MigrationPage: React.FC = () => {
    ...
    const type = searchParams.get('type'); // 'current' or 'new'
            ...
            if (result.success) {
                ...
                if (result.currentEmailVerified && result.newEmailVerified) {
                    ...
                    if (result.needsFinalization || result.hasPendingMigration) {
                        ...
                        try {
                            const finalizeResult = await api.request('/auth/migrate/finalize', {
                                method: 'POST',
                            });
    ...
    const handleEmailVerification = async (t: string, verificationType: string) => {
        ...
        try {
            const endpoint = verificationType === 'current' ? '/auth/migrate/verify-current' : '/auth/migrate/verify-new';
            ...
            if (result.success) {
                setStatus({
                    loading: false,
                    error: '',
                    success: result.message || 'Email verified successfully!',
                });
                
                // Immediately refresh migration status to show updated verification state
                ...
                // Handle redirect based on account existence or URL parameter
                if (verificationType === 'new') {
                    const redirect = searchParams.get('redirect');
                    const shouldRedirectToLogin = redirect === 'login' || (result.accountExists && result.redirectTo === 'login');
                    const shouldRedirectToSignup = redirect === 'signup' || (!result.accountExists && result.redirectTo === 'signup');
                    
                    // CRITICAL: If both emails are verified, migration is complete - redirect to login
                    if (result.bothVerified || (updatedStatus?.currentEmailVerified && updatedStatus?.newEmailVerified)) {
                        // Migration complete - redirect to login with new email
                        setTimeout(() => {
                            navigate('/login?email=' + encodeURIComponent(result.newEmail || updatedStatus?.newEmail || migrationStatus?.newEmail || ''));
                        }, 2000);
                        ...
                        setTimeout(() => {
                            navigate('/register?email=' + encodeURIComponent(result.newEmail || updatedStatus?.newEmail || migrationStatus?.newEmail || ''));
                        }, 2000);
                    }
                }
                ...
                setStatus({ loading: false, error: result.message || 'Verification failed', success: '' });
            }
        } catch (error: any) {
            setStatus({ loading: false, error: error.message || 'Verification failed', success: '' });
        } finally {
    ...
    // Check migration status once when component mounts (only if no token/type in URL)
    useEffect(() => {
        if (user && !token && !type) {
    ...
    const handleSubmit = async (e: React.FormEvent, confirmOverride: boolean = false) => {
        ...
        try {
            ...
            if (result.success) {
                ...
                if (result.code === 'EMAIL_EXISTS' && result.requiresConfirmation) {
                    setEmailExistsWarning({
                        exists: true,
                        existingAccountInfo: result.existingAccountInfo
                    });
                    setStatus({ 
                        loading: false, 
                        error: result.warning || result.message, 
                        success: '' 
                    });
                } else {
            ...
            if (error.code === 'EMAIL_EXISTS' && error.requiresConfirmation) {
                setEmailExistsWarning({
                    exists: true,
                    existingAccountInfo: error.existingAccountInfo
                });
                setStatus({ 
                    loading: false, 
                    error: error.warning || error.message, 
                    success: '' 
                });
            } else {
    ...
    useEffect(() => {
        if (token && type && (type === 'current' || type === 'new')) {
                ...
                {emailExistsWarning?.exists && (
                    <div style={{
                        ...
                        <p style={{ marginBottom: 'var(--spacing-2)' }}>
                            ...
                            This account is associated with <strong>{emailExistsWarning.existingAccountInfo?.displayName}</strong>
                            (Provider: {emailExistsWarning.existingAccountInfo?.provider}, Created: {new Date(emailExistsWarning.existingAccountInfo?.createdAt || '').toLocaleDateString()}).
                        </p>
                        {emailExistsWarning.existingAccountInfo?.hasBio && (
                            <p style={{ marginBottom: 'var(--spacing-2)', fontWeight: 'bold' }}>
                                This existing account has a bio. Proceeding will delete this existing account and its data.
                            </p>
                        )}
