...
const finalizeMigrationInternal = async (user: IUser, res: Response): Promise<void> => {
    try {
        ...
        // CRITICAL: Check if a user with the new email already exists
        ...
        if (userWithTokens) {
            userWithTokens.refreshTokens = [...(userWithTokens.refreshTokens || []).slice(-4), refreshToken];
            await userWithTokens.save();
        }

        // CRITICAL: Get final user data to return - ensure ALL fields including bio are loaded
        // Use explicit select to ensure bio is included
        ...
        // Log bio status for debugging
        console.log(`📊 Final user data after migration:`);
        console.log(`   Email: ${finalUser.email}`);
        console.log(`   Bio: ${finalUser.bio || 'MISSING - THIS IS A PROBLEM!'}`);
        console.log(`   Display Name: ${finalUser.displayName}`);
        console.log(`   Avatar: ${finalUser.avatar || 'None'}`);
        console.log(`   Previous Email: ${finalUser.previousEmail || 'None'}`);
        
        // CRITICAL: Double-check bio was preserved - if missing, log warning
        if (!finalUser.bio) {
            console.error(`⚠️  WARNING: Bio is missing after migration! This should not happen if bio existed before migration.`);
        }
        ...
        res.json({
            ...
            user: {
                ...
                bio: finalUser.bio || '', // CRITICAL: Bio must be preserved - return empty string if missing
        ...
        if (error.code === 11000 && error.keyPattern?.email) {
            ...
            try {
                ...
                if (duplicateEmail) {
                    const duplicateUser = await User.findOne({ email: duplicateEmail });
...
export const initiateMigration = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Migration cooldown removed for testing (set to 0 = no limit)
        // No cooldown check - users can migrate immediately
        ...
        console.log(`🔍 Pre-migration check: Account exists for ${newEmail}: ${accountExists ? 'YES' : 'NO'}`);
        
        if (existing) {
            // If it's the same user (shouldn't happen, but check anyway)
            ...
            console.log(`⚠️  Account exists! Display Name: ${existing.displayName}, Has Bio: ${!!existing.bio}`);
            
            // If user hasn't confirmed override, return warning with data override details
            if (!confirmOverride || confirmOverride !== 'true') {
                console.log(`⚠️  Returning EMAIL_EXISTS warning - user needs to confirm BEFORE migration starts`);
                res.status(409).json({ 
                    success: false, 
                    ...
                    warning: `⚠️ WARNING: An account with email "${newEmail}" already exists!\n\n` +
                            `If you proceed:\n` +
                            `• The existing account (${existing.displayName || existing.email}) will be PERMANENTLY DELETED\n` +
                            `• All data from the existing account (bio, posts, settings, etc.) will be LOST\n` +
                            `• Your current account data (bio, display name, avatar, etc.) will be migrated to this email\n\n` +
            ...
            // User confirmed - proceed with migration (existing account will be deleted during finalization)
            console.log(`✅ User confirmed override. Existing account ${existing._id} will be deleted during migration.`);
        } else {
            console.log(`✅ No existing account found - new account will be created during verification`);
        }
        ...
        if (accountExists) {
            // Account exists - redirect to login page after verification
            newEmailRedirectUrl = `${config.clientUrl}/migrate?token=${newEmailToken}&type=new&redirect=login`;
        } else {
            // Account doesn't exist - redirect to signup page after verification
            newEmailRedirectUrl = `${config.clientUrl}/migrate?token=${newEmailToken}&type=new&redirect=signup`;
        }

        // Send email to CURRENT email
        const currentVerifyUrl = `${config.clientUrl}/migrate?token=${currentEmailToken}&type=current`;
        ...
        res.json({ 
            ...
            accountExists: accountExists // Tell frontend if account exists for redirect
        });
    } catch (error) {
...
export const verifyCurrentEmail = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!token) {
            console.error('❌ Verify Current Email: No token provided');
...
export const verifyNewEmail = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        if (!token) {
            console.error('❌ Verify New Email: No token provided');
            res.status(400).json({ success: false, message: 'Token is required' });
            return;
        }
        ...
        const newEmail = user.newEmailPending!.toLowerCase();
        let existingNewEmailUser = await User.findOne({ email: newEmail });
        ...
        if (!accountExists) {
            ...
            // Reload to get the new account
            existingNewEmailUser = await User.findOne({ email: newEmail });
        }
        ...
        res.json({ 
            ...
            newEmailVerified: true,
            bothVerified: false,
            accountExists, // Tell frontend if account exists for redirect (already calculated above)
            redirectTo: accountExists ? 'login' : 'signup',
...
export const resendMigrationEmails = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        // Check if new email account already exists to determine redirect
        const existingNewEmailUser = await User.findOne({ email: userDoc.newEmailPending!.toLowerCase() });
        const newEmailExists = !!existingNewEmailUser;
        
        // Determine redirect URL for new email based on whether account exists
        let newVerifyUrl = `${config.clientUrl}/migrate?token=${newEmailToken}&type=new`;
        if (newEmailExists) {
            newVerifyUrl = `${config.clientUrl}/migrate?token=${newEmailToken}&type=new&redirect=login`;
        } else {
            newVerifyUrl = `${config.clientUrl}/migrate?token=${newEmailToken}&type=new&redirect=signup`;
        }
